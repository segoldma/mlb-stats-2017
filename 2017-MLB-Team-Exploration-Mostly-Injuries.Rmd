...
Although the Washington Nationals have crashed out of the MLB playoffs for the [fourth time in four appearances](http://www.espn.com/blog/washington-nationals/post/_/id/3300/curses-nationals-playoff-dreams-are-foiled-again) since 2012, I'm not ready to put the 2017 season to bed. While practicing exploratory data analysis and plotting with [ggplot2](http://ggplot2.tidyverse.org/reference/), I downloaded a couple of datasets from the 2017 MLB Regular Season to play around with:
...
As I started to examine the tables, I had to return to bbref to look up the `#Bat` and `#P` variables, while represent the number of distinct *batters* and *pitchers* that appeared for each team.
...
Since pitchers have to bat in the National League, I'd expect NL teams to have 4-5 more batters on average than the AL. After plotting the two leagues side-by-side, my hunch was disproved. I hadn't considered interleague games, which reverse the effect.
...
#### Does this mean that using fewer batters tend to win more games?
...
I anticipated the relationship to be stronger in the AL, with CLE, HOU, and BOS using the fewest batters in the AL, while combining for 296 wins. However, I wonder if these highly successful teams won more games with fewer players because they managed to avoid injuries, or simply better individual hitters among on their rosters.

To examine this, I found [MLB Disabled List](http://www.spotrac.com/mlb/disabled-list/) data on [Sportrac](http://www.spotrac.com/). This dataset includes each team's number of distinct players who spent time on MLB's Disabled List, along with the corresponding number of days. It also includes summaries for `batters` as well as `pitchers`, previewed below:
...
```{r batter_injury_corrplot, eval = TRUE}
...
```
```{r batter_injury_corrplot_code, ref.label = "batter_injury_corrplot", eval = FALSE, echo = TRUE}
```
...
* number of batters used is slightly negatively correlated with Team Wins (although we found earlier that the relationship is stronger in the `AL` than `NL`.)

**A bit more surprising:** 

* there seems to have been little-to-no relationship between a team's number of batters on the DL, and Team Wins `(-0.05)`
* nor is their much of any relationship between the number of days batters spent on the DL and team wins `(0.03)`

#### Did the number of batters a team uses relate to offense production?
